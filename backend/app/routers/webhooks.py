...
def _verify_whatsapp_baileys(channel: MessagingChannel, request: Request, raw_body: bytes) -> bool:
    ...
    in the X-Bridge-Secret header. Fail closed: a channel without a secret
    rejects every request (401) instead of accepting unsigned posts — the
    endpoint would otherwise be open to anyone who can reach the backend
    (message injection + replies sent from the user's WhatsApp number).
    Channels created before secrets were generated by default get their
    secret backfilled on the first QR open (see get_baileys_qr).
    """
    if not channel.webhook_secret:
        log.error("baileys channel %s has no webhook_secret — rejecting request", channel.id)
        return False
