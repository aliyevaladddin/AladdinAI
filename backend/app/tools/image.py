...
async def generate_image(ctx: ToolContext, prompt: str) -> dict:
    ...
    try:
        if channel_type == "telegram":
            ...
            # send_telegram_photo uploads from a real on-disk path, but the
            # media store may be GridFS (no file on disk — the handle is an
            # ObjectId, which is exactly the "file not found" this used to
            # hit). The generated bytes are already in memory, so materialise
            # them into a temp file for the duration of the upload.
            suffix = os.path.splitext(saved["filename"])[1] or ".png"
            tmp = tempfile.NamedTemporaryFile(suffix=suffix, delete=False)
            try:
                tmp.write(img_bytes)
                tmp.close()
                await send_telegram_photo(channel, str(recipient), tmp.name, caption=prompt)
            finally:
                try:
                    os.unlink(tmp.name)
                except OSError:
